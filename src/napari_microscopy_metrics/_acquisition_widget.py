...
class Acquisition_tool_page(QWidget):
    ...
    def __init__(self, viewer: "napari.viewer.Viewer"):
        ...
        self.lbl_pixel_size.setSizePolicy(
            QSizePolicy.Preferred, QSizePolicy.Fixed
        )
        self.options_PxS = Options("Pixel size", "Median Filter")
        self.options_PxS.addFloat(
            name="Pixel size X", value=self.params["PhysicSizeX"]
        )
        self.options_PxS.addFloat(
            name="Pixel size Y", value=self.params["PhysicSizeY"]
        )
        self.options_PxS.addFloat(
            name="Pixel size Z", value=self.params["PhysicSizeZ"]
        )
        ...
        self.title_options_microscope.setSizePolicy(
            QSizePolicy.Preferred, QSizePolicy.Fixed
        )
        ...
        self.options_microscope.addInt(
            ...
            value=self.params["Emission_Wavelength"],
        )
        self.options_microscope.addFloat(
            name="Refraction index", value=self.params["Refractive_index"]
        )
        self.options_microscope.addFloat(
            name="Numerical aperture", value=self.params["Numerical_aperture"]
        )
        self.widget_micro_choice = OptionsWidget(
            self.viewer, self.options_microscope
        )
    ...
    def _on_apply(self):
        ...
        for i in range(len(self.viewer.layers)):
            ...
            self.viewer.layers[i].scale = [
                self.params["PhysicSizeZ"],
                self.params["PhysicSizeY"],
                self.params["PhysicSizeX"],
            ]

    def _on_layer_changed(self):
        """updating image shape values when changing layer"""
        current_layer = self.viewer.layers.selection.active
        if current_layer is None or not isinstance(
            current_layer, napari.layers.Image
        ):
            return
    ...
    def update_params(self):
        ...
        self.params["Emission_Wavelength"] = self.options_microscope.value(
            "Emission wavelength"
        )
        self.params["Microscope_type"] = self.options_microscope.value(
            "Microscope type"
        )
        self.params["Numerical_aperture"] = self.options_microscope.value(
            "Numerical aperture"
        )
        self.params["Refractive_index"] = self.options_microscope.value(
            "Refraction index"
        )
